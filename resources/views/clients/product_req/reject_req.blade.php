...
@section('statistics')
                                        ...
                                        <tbody>
                                            ...
                                            <tr class="gradeC" style="font-family: b koodak;">
                                                <td style="text-align: center;">
                                                    <a href="#" class="btn btn-warning btn-sm btn-line" data-toggle="modal" data-target="#delete{{ $prequest->id }}">حذف</a>                                                
                                                    <a href="<?=Url('client/product_req/'.$prequest->id); ?>" class="btn btn-info btn-sm btn-line">بازبینی</a>
                                                </td>

                                                   <td style="text-align: center;"><span style="color: black;">{{ $prequest->reject_desc }}</span></td>
                                                    
                                                   <td style="text-align: center;">{{ jdate($prequest->reject_date)->format('%B %d، %Y') }}</td>
                                                    
                                                    @if( $prequest->reject_code == '1' )
                                                        <td style="color: black;text-align: center;"> مسئول واحد تدارکات</td>
                                                    @elseif( $prequest->reject_code == '2' )
                                                        <td style="color: black;text-align: center;"> تأیید کننده تدارکات </td>
                                                    @elseif( $prequest->reject_code == '3' )
                                                        <td style="color: black;text-align: center;"> مسئول اعتبار</td>
                                                    @elseif( $prequest->reject_code == '4' )
                                                        <td style="color: black;text-align: center;"> مدیر امور مالی</td>
                                                    @elseif($prequest->reject_code == '5' )
                                                        <td style="color: black;text-align: center;"> تصویب کننده </td>                                   
                                                    @endif                                                
                                                
                                                <td style="text-align: center;"> {{ jdate($prequest->created_at)->format('%B %d، %Y') }}  </td>
                                                <td style="text-align: center;"> {{ $prequest->codepaygiry }} </td>                                              
                                            </tr>
                                            @endforeach 
                                        </tbody>
                                    </table>
      ...
      <div class="row">
          <div class="col-lg-12">
              @foreach( $prequests as $prequest )
                <div class="modal fade" id="delete{{ $prequest->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                                <h4 class="modal-title" id="H2" style="direction: rtl; font-family: b koodak;"> آیا از حذف درخواست {{ $prequest->codepaygiry }}  اطمینان دارید ؟ </h4>
                            </div>
                            ...
                            <div class="modal-footer">                        
                                <form action="<?= Url('client/prequest/destroy/'.$prequest->id); ?>" method="POST">
