...
@section('statistics')
	        <div class="block">
                            <div class="navbar navbar-inner block-header">
                                <div class="muted pull-right" style="font-family: b koodak;">لیست درخواست های خدمت انصرافی تأیید شده</div>
                                ...
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                        <thead>
                                            <tr style="font-family: b koodak;">
                                                ...
                                                <th style="text-align: center;"> تاریخ موافقت با انصراف </th>   
                                                <th style="text-align: center;"> تاریخ ثبت انصراف </th>
                                                <th style="text-align: center;"> وضعیت درخواست </th>
                                                <th style="text-align: center;"> تاریخ ثبت درخواست خدمت</th>
                                        ...
                                        <tbody>
                                            ...
                                            <tr class="gradeC" style="font-family: b koodak;">
                                                ...
                                                <a href="{{ route('cconfirm1' , ['id' => $cancel_confirm->id]) }}" class="btn btn-info btn-sm btn-line">مشاهده جزئیات</a>
                                                </td>

                                                <td style="text-align: center;"> {{ jdate($cancel_confirm->cancel_confirm_date)->format('%B %d، %Y') }}</td>

                                                <td style="text-align: center;"> {{ jdate($cancel_confirm->cancel_date)->format('%B %d، %Y') }}</td>
                                                   
                                                    @if( $cancel_confirm->state_req == '0' )
                                                        <td style="color: black;text-align: center;">این درخواست تازه ثبت شده اما انصراف داده شده است </td>
                                                    @elseif( $cancel_confirm->state_req == '1' )
                                                        <td style="color: black; text-align: center;">این درخواست توسط مسئول واحد تأیید شده اما انصراف داده شده است</td>
                                                    @elseif( $cancel_confirm->state_req == '2' )
                                                        <td style="color: black;text-align: center;">این درخوسات ، مجوز تأیید کننده را دریافت نموده اما انصراف داده شده است  </td>
                                                    @elseif( $cancel_confirm->state_req == '3' )
                                                        <td style="color: black;text-align: center;">این درخواست توسط مسئول اعتبار تأیید شده اما انصراف داده شده است</td>
                                                    @elseif( $cancel_confirm->state_req == '4' )
                                                        <td style="color: black;text-align: center;">این درخواست توسط امور مالی تأیید شده اما انصراف داده شده است</td>
                                                    @elseif( $cancel_confirm->state_req == '5' )
                                                        <td style="color: black;text-align: center;">این درخواست ، مجوز تصویب کننده را دریافت نموده اما انصراف داده شده است</td>                                     
                                                    @endif
