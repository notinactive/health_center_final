...
@section('statistics')
	        <div class="block">
                             ...
                             <br>
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                        ...
                                        <tbody>
                                            ...
                                            @foreach( $notseens as $notseen )
                                            <tr class="gradeC" style="font-family: b koodak;">
                                                <td style="text-align: center;">                               
                                                <a href="#" class="btn btn-info btn-sm btn-line" data-toggle="modal" data-target="#delete{{ $notseen->id }}">مشاهده جزئیات</a>
                                                </td>
                                                 
                                                 @if( $notseen->olaviat == '0' )
                                                        <td style="color: red; text-align: center;">اولویتی انتخاب نشده است</td>
                                                 @elseif( $notseen->olaviat == '1' )
                                                        <td style="color: black; text-align: center;"> اولویت کم</td>
                                                 @elseif( $notseen->olaviat == '2' )
                                                        <td style="color: black; text-align: center;"> اولویت متوسط</td>
                                                 @elseif( $notseen->olaviat == '3' )
                                                        <td style="color: black; text-align: center;"> اولویت زیاد</td>       
                                                 @endif 

                                                <td style="text-align: center;"> {{ $notseen->title }} </td>
                                                <td style="text-align: center;"> {{ jdate($notseen->created_at)->format('%B %d، %Y') }} </td>
                                                <td style="text-align: center;"> {{ $notseen->codepaygiry }} </td>                                              
                                            </tr>
                                            @endforeach 
                                        </tbody>
                                    </table>
                    ...
                    <div class="row">
                        <div class="col-lg-12"> 
                          @foreach( $notseens as $notseen )                           
                            <div class="modal fade" id="delete{{ $notseen->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                                            <h4 class="modal-title" id="H2" style="font-family: b koodak;"><span style="color:blue; direction: rtl; text-align: justify;">"{{ $notseen->codepaygiry }}" </span>جزئیات تیکت با کد پیگیری </h4>
                                        </div>
                                        <div class="modal-body">                    
                                          ...
                                          <span style="font-family: b koodak; direction: rtl; float: right;"> تاریخ ثبت : {{ jdate($notseen->created_at)->format('%B %d، %Y') }}</span>                                         
